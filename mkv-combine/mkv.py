    ...
    def verify_mkvmerge(self, mkvmerge_path=None):
        ...
        try:
            ...
            raise ExternalInstallError(
                f"MKVMerge installed at {_mkvmerge_path} has not passed the install verification", program_name=_mkvmerge_path
            )
...
class MKVTrack(MKV):
    def __init__(
        self, file_path, mkvmerge_path=None, track_id=0, track_name=None, language=None, default_track=False, forced_track=False
    ) -> None:
...
class MKVFile(MKV):
    def __init__(self, file_path, mkvmerge_path=None, title: Optional[str] = None) -> None:
        super().__init__(file_path, mkvmerge_path)
...
if __name__ == "__main__":
    mkv_file = MKVFile(
        "Insert path to video file here",
    )
    mkv_file.add_track(
        "Insert path to subtitle file here",
    )
